            ...
            <div className="flex flex-col gap-4 max-w-4xl md:p-3 rounded-md">
                ...
                <div className="flex items-center justify-between w-fit flex-wrap gap-2">
                    ...
                    <div className="flex flex-col items-start gap-0">
                        <h3 onClick={() => navigate(`/dashboard/discussion/profile/${discussion?.createdBy._id}`)} className="font-semibold text-gray-800 hover:text-teal-600 cursor-pointer uppercase tracking-wide text-sm">
                            {discussion?.createdBy?.name}
                        </h3>
                ...
                <div>
                    <h1 className="md:text-3xl uppercase text-teal-600 text-xl font-semibold">{discussion?.title}</h1>
                    <p className="text-md md:text-xl py-1 text-gray-500 md:max-w-4xl">{discussion?.description}</p>
                </div>
                <div>
                    <p className="rounded-md flex items-center text-lg text-gray-600">Hi, <span className="text-teal-600 px-2 font-bold uppercase">{user?.name}</span></p>
                ...
                <form onSubmit={handleSubmit} className="flex gap-3 flex-col items-start justify-center w-full">
                    ...
                    <button type="submit" disabled={loading} className="bg-teal-600 hover:bg-teal-700 cursor-pointer text-white px-6 py-2 rounded-sm transition-all disabled:opacity-50">
                        {loading ? ("Sending...") : (
